...
def render_policy(workspace: ProjectSnapshot, synced_at: str | None = None) -> str:
    ...
    lines = [
        ...
        "6. Copy the applicable rule IDs and matched idea policy into `PLAN.md` before preserving `PLAN.initial.md` and starting a worker. The policy boundaries stay fixed; the worker may adapt implementation details and record the final method.",
    ...
    lines.extend(
        [
            ...
            "",
            "Only this active generated file controls future cycles. Saved policy boundaries for running work do not change mid-run, but the implementation plan may adapt.",
            "",
        ]
    )
    return "\n".join(lines)
...
def render_loop_instructions(workspace: ProjectSnapshot, synced_at: str | None = None) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Before preserving `PLAN.initial.md`, add a `## Delta Loop Policy` section to the test brief containing:",
            "",
            ...
            "",
            "Copy that exact policy section into the worker handoff. The policy and scientific boundaries remain fixed even if the working method changes later.",
            "",
            "## Plan changes and result meaning",
            "",
            "The saved plan is a starting implementation, not the object being tested. The worker may revise code, commands, methods, and intermediate steps to produce a valid test, as long as it preserves the intended idea, fair-comparison meaning, measurement, resource limits, and hard prohibitions. Record the final method and meaningful adaptations for audit. Do not count a modified plan as a failure or as evidence against an idea.",
            "",
            "One research run represents one attempt to answer a scientific question, not one shell command. If a command, dependency, path, or implementation is wrong, repair it with `delta work retry RUN_ID --command ... --reason ...` so it remains inside the same run and output history. Do not call `delta work start` again for a minor edit or debugging retry. Create a new run only for a materially different idea, comparison, or measurement, or after the current run has produced reviewed evidence.",
            "",
            "Review only after the run produces usable evidence or reaches a genuine hard boundary after reasonable repairs. Record two separate judgments with `delta work review`: (1) whether the final execution is valid, partly valid, invalid, or unsure; and (2) whether trustworthy evidence supports the idea, challenges it, is inconclusive, is invalid, or is not applicable. Only a valid scientific result may support or challenge the idea. A command error or unusable intermediate try is not a completed research result.",
            "",
            "## When to stop",
            "",
            ...
            "## Persistent terminal",
            "",
            "When `DELTA_LOOP_WORKSPACE_ID` is set, continue immediate cycles in this same supervisor terminal. A bounded worker may be started from the saved test brief, but do not start a fresh supervisor merely to continue the loop.",
            "",
            "## Which rule wins",
            "",
            "Required safety rule → matching idea policy → other enabled rules → the active research-loop step.",
